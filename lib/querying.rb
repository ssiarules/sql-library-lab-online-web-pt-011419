...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters ON series.id = characters.series_id GROUP BY species ORDER BY COUNT(species) LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, FROM characters JOIN character_books ON characters.id = character_books.character_id, character_id = character_books.book_id DESC limit 1; "
end
